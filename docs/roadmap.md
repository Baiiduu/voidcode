...
**当前状态：** 进行中。CLI 仍是最完整的客户端入口；TUI 已有初始实现，但还需要补齐会话管理与自动化冒烟验证；Web 客户端已经接入极简的真实运行时路径，但仍需继续打磨为更完整的运行时驱动体验。
...
当 VoidCode 能够可靠地演示一个受监管的单智能体开发任务流，并具备持久化、审批、可观测性以及至少一个可用的客户端入口点时，即认为达到了 MVP 边界。
