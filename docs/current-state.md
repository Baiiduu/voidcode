...
本文档提供了 VoidCode 仓库截至 2026 年 4 月的真实快照。VoidCode 目前处于 **pre-MVP 后期阶段，已经具备稳定的单智能体循环**。
...
**当前集成状态**：🟡 **客户端层已经打通最小可用路径**。CLI 是目前最完整的入口；TUI 已有初始实现；Web 前端已经接通本地 HTTP/SSE 传输，并消费会话列表、会话重放、流式运行和审批处理路径，但整体交互体验仍然停留在 pre-MVP 水平。
...
- [~] **TUI 客户端**：初始实现已落地，当前仍缺少更完整的会话管理、规范冒烟测试以及与规范的进一步对齐。
...
### 计划中 / 进行中
- [~] **实时 API 集成**：针对极简传输，前端已经接入会话列表、会话重放、流式运行事件与审批处理；更完整的运行时驱动任务体验仍待继续实现。
