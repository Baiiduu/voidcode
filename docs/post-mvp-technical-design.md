...
进入 WIP 阶段之后，VoidCode 的目标不再只是“有一个稳定可演示的确定性 execution engine”，而是向一个**本地优先、运行时中心、可恢复、可扩展**的真实 coding agent runtime 演进。

本阶段的设计目标是：

1. 保持当前运行时边界稳定，不推倒重来。
2. 在现有运行时内部引入一个**真实的、可恢复的、provider-backed execution engine**。
3. 将 skill、LSP、ACP、provider 这些现在还停留在发现/配置/存根层的能力，提升为运行时可管理的能力面。
4. 继续强化本地持久化、事件流、审批与恢复，而不是让客户端承担执行状态。
5. 为后续可能出现的多角色执行拓扑 / ACP 控制面预留边界，但不把它们作为当前 WIP 的前置条件。
...
- 不立即把系统扩展成真实 multi-agent / multi-role 平台。
...
3. **Execution engines**
   ...
   - 新增一个 provider-backed execution engine，作为首个真实 WIP engine
...
这使得系统能够在不破坏已有客户端和 session 语义的前提下，从 MVP 的 deterministic engine 演进到真实的 provider-backed execution。
...
- `agent`：新的 provider-backed execution engine
...
这一能力可以先限定在单一执行路径中完成，不要求同时引入 subagent 或更复杂的多角色拓扑。
...
ACP 在当前阶段不应被理解为“必须马上做 multi-agent”。更合理的定位是：
...
进一步说，checkpoint 在这里必须被视为 **resume anchor**，而不只是给客户端展示的 UI summary：它至少要能够支撑 approval resume、transport replay 之后的继续执行，以及 provider-backed execution 路径在 compaction 之后的恢复。
...
### Wave 1：Provider-backed Execution Engine

这是 post-MVP 的**最高优先级 WIP 切片**。

目标：在现有 runtime 边界内部，新增一个真实的 provider-backed execution engine。
...
- 一个真实的 provider-backed execution run 可以在 provider 支撑下完成
- 工具调用仍通过 runtime 治理
- 审批与恢复保持成立
- session replay 仍然可用
...
### WIP-1：Provider-backed Execution 闭环

必须完成：

- runtime 内的 engine selection
- 一个 provider-backed execution engine
- provider 抽象的最小闭环
...
当前运行时在真实 provider 接入后会面临上下文持续增长的问题；如果没有窗口裁剪、压缩或 summarization 触发点，provider-backed execution 路径很快会遇到上下文溢出或成本失控。
...
当前 provider-backed execution engine 只是最小闭环，尚未具备真实 provider 接入后的容错能力。下一步必须补齐：
...
当前 provider-backed execution path 的 step 上限不应长期硬编码。将其纳入 `RuntimeConfig` 或 request 级覆盖可以较低成本提升灵活性，并让不同任务类型拥有更合理的执行预算。

#### 4. 流式执行模型从同步 `Iterator` 演进到异步流

当前同步 `Iterator` 路径足以支撑 MVP 与首个 provider-backed slice，但在真实 provider 流式输出、并发工具执行和更丰富的 transport 场景下，异步流模型会更合理。

这项工作应在真实 provider 路径稳定之后推进，而不是与首个 provider-backed execution engine 同时耦合实现。
...
当前 deterministic 与 provider-backed execution slice 都还比较轻量，因此长期需要重新评估 LangGraph 是否继续提供足够价值。该评估应以实际复杂度为准，而不是预先假设“必须保留”或“必须移除”。

评估标准应包括：

- 当前 graph 层是否已经承担真实状态机复杂度
- plain async loop 是否能更直接表达单角色执行模型
...
在 provider-backed execution path、checkpoint groundwork 和配置持久化语义已经落地之后，下一批更适合直接启动的 issue 应收敛到以下几类：
...
- 新增一个真实的 provider-backed execution engine
